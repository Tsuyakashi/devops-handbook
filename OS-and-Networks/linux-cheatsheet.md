# Foundation & OS:

- Linux: Ubuntu (base), RHEL/AlmaLinux (vs old CentOS)
- bash (scripting, grep/sed/awk)
- virtulization (kvm, qemu) + vagrant
- system management: systemd - services, cron, sudo/chmod/chown, systemd-journald, logrotate

# Bash scripting

## grep
```
grep [флаги] 'шаблон' [имя_файла]
```

### Полезные флаги
- -i — игнорировать регистр символов.
- -v — инвертировать поиск (вывести строки, которые не содержат шаблон).
- -r или -R — рекурсивный поиск по всем подпапкам (очень полезно для поиска по исходному коду).
- -n — вывести номер строки, где найден результат.
- -c — вывести только количество найденных совпадений.
- -l — вывести только имена файлов, где найдено совпадение.
- -w — искать совпадение только как целое слово (чтобы не находить части других слов).

### Основы регулярных выражений
^ — начало строки (например, ^start найдет строки, начинающиеся на start).
$ — конец строки (например, end$ найдет строки, заканчивающиеся на end).
. — любой одиночный символ.
.* — любое количество любых символов.

## sed

```
sed [флаги] 'команда' имя_файла
```

### Главные флаги

- -i — изменить файл напрямую (без этого флага sed только выводит результат на экран).
- -E — включить поддержку расширенных регулярных выражений (ERE).
- -n — подавить автоматический вывод строк (используется вместе с командой p).

1. Замена текста (Команда s)

    - `sed 's/apple/orange/' file.txt` — заменить первое совпадение apple на orange в каждой строке.
    - `sed 's/apple/orange/g'` — заменить все совпадения apple на orange (глобально).
    - `sed 's/apple/orange/2'` — заменить только второе совпадение в строке.
    - `sed 's/apple/orange/I'` — заменить без учета регистра (apple, Apple, APPLE).
    - `sed -i 's/old/new/g' file.txt` — перезаписать файл с заменой.

2. Удаление строк (Команда d)

    - `sed '3d' file.txt` — удалить 3-ю строку.
    - `sed '1,5d' file.txt` — удалить строки с 1 по 5.
    - `sed '$d' file.txt` — удалить последнюю строку.
    - `sed '/pattern/d' file.txt` — удалить все строки, содержащие pattern.
    - `sed '/^$/d' file.txt` — удалить все пустые строки.

3. Печать строк (Команда p)

    - `sed -n '5p' file.txt` — напечатать только 5-ю строку.
    - `sed -n '2,10p' file.txt` — напечатать строки с 2 по 10.
    - `sed -n '/pattern/p' file.txt` — напечатать только строки с pattern (аналог grep).

## awk

```
awk '[условие] {действие}' имя_файла
```

### Переменные полей (столбцов)

По умолчанию разделителем является пробел или знак табуляции.

- $0 — вся строка целиком.
- $1, $2, $3 — первое, второе, третье поле (столбец) строки.
- $NF — последнее поле строки (удобно, когда число столбцов меняется).

### Встроенные переменные

- FS — разделитель полей на входе (по умолчанию пробел).
    
- OFS — разделитель полей на выходе (по умолчанию пробел).
    
- NR — текущий номер строки (порядковый номер записи).
    
- NF — общее количество полей в текущей строке.

### Главный флаг

- -F — задать свой разделитель полей (например, -F':' для файлов конфигурации вроде /etc/passwd или -F',' для CSV).

### Примеры частых операций

1. Вывод конкретных столбцов (Печать)
    - `awk '{print $1, $3}' file.txt` — вывести первый и третий столбцы.
    - `awk -F':' '{print $1, $NF}' /etc/passwd` — использовать : как разделитель, вывести имя пользователя и его оболочку (последнее поле).
    - `awk '{print NR, $0}' file.txt` — пронумеровать и вывести все строки.

2. Фильтрация и условия (Поиск)
    - `awk '/pattern/ {print $2}' file.txt` — найти строки с pattern и вывести их второй столбец.
    - `awk '$3 > 100' file.txt` — вывести строки, где значение в третьем столбце больше 100.
    - `awk 'NF == 4' file.txt` — вывести только те строки, которые состоят ровно из 4 столбцов.
    - `awk 'NR>=2 && NR<=5' file.txt` — вывести строки с 2 по 5 (аналог sed -n '2,5p').

3. Блоки BEGIN и END (Подсчет и математика)
    - `BEGIN` выполняется до чтения текста, `END` — после обработки всех строк.
    - `awk '{sum += $1} END {print sum}' file.txt` — посчитать сумму всех чисел в первом столбце.
    - `awk 'END {print NR}' file.txt` — посчитать общее количество строк в файле.

# Ubuntu

В контексте AWS уже есть опыт взаимодействия с Ubuntu. Ubuntu дефакто является стандартом в серверной части. Так же удобен в использовании как и десктопная OS (по моему мнению потому что достаточно популярна к сообществе, в следствии очень присособлена в использовании юзером)

## Основные разделения по дистрибутивам Ubuntu:

- ### Desktop
    <3

- ### WSL: 
    Удобный кейс при необходимости использования, когда основная OS - windows, имхо: базовая оболочка powershell портит опыт взаимодействия (как минимум после удобства ctrl+alt+T и прочих мини фич), + неприятные шаманские танцы при необходимости подключить виртуализацию, в остальном крайне достойная и бескомпромиссная тема

- ### Server:
    На момент написания 26.04 ласт ЛТС версия, базирующаяся на Linux 7.0 Kernel

    - LTS - Long Term Support 
    
- ### Cloud:
    Оптимизированная версия под AWS, Azure, Google Cloud

    Содержит спец утилилиту (как и в сереверной версии) - cloud-init для автоматической настройки при старте 
    
- ### Ubuntu Core:
    это максимально урезанная, полностью изолированная и неизменяемая (immutable) версия ОС. Она создана для IoT-устройств, робототехники и встроенных систем (Edge). Все программы и сама система в ней работают через безопасные пакеты Snap.

- ### Ubuntu Flavors (дистрибутивы-«вкусы»):
    это официальные сборки Ubuntu Desktop, но с другими графическими оболочками и набором встроенных программ вместо стандартного GNOME (например, Kubuntu с KDE, Xubuntu с XFCE, Lubuntu с LXQt).


    