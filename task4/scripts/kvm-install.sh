...
function initKvm() {
    ...
    virt-install \
        ...
        --memory $VM_MEMORY \
        --vcpus $VM_CPUS \
        --disk path=/var/lib/libvirt/images/$VM_IMAGE,format=img \
...
if [[ "$VM_OS" == "ubuntu" ]]; then
    ...
    VM_IMAGE_LINK=https://cloud-images.ubuntu.com/noble/20251126/noble-server-cloudimg-amd64.img
    echo "ERROR: old download link" && exit 1
else
